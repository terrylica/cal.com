...
describe("Bookings Endpoints 2024-08-13 update booking location", () => {
  ...
  describe("PATCH /v2/bookings/:bookingUid/location", () => {
    ...
    describe("videoCallUrl metadata updates", () => {
      it("should update videoCallUrl metadata when location is updated to a URL", async () => {
        const initialLocation = "123 Main St";
        const newLink = "https://meet.example.com/video-call-123";
        const { bookingUid } = await createBooking(initialLocation);
        ...
        const updatedBooking = responseBody.data as BookingOutput_2024_08_13;
        expect(updatedBooking.location).toEqual(newLink);
        expect(updatedBooking.metadata?.videoCallUrl).toEqual(newLink);
      });

      it("should clear videoCallUrl metadata when location is updated to a non-URL value", async () => {
        const initialLink = "https://meet.example.com/initial-call";
        const newAddress = "456 Oak Street, Suite 100";
        const { bookingUid } = await createBooking(initialLink);
        ...
        const updatedBooking = responseBody.data as BookingOutput_2024_08_13;
        expect(updatedBooking.location).toEqual(newAddress);
        expect(updatedBooking.metadata?.videoCallUrl).toEqual("");
      });

      it("should update videoCallUrl metadata when changing from one URL to another", async () => {
        const initialLink = "https://meet.example.com/old-call";
        const newLink = "https://zoom.us/j/new-meeting-id";
        const { bookingUid } = await createBooking(initialLink);
      ...
      it("should clear videoCallUrl metadata when location is updated to a phone number", async () => {
        const initialLink = "https://meet.example.com/video-call";
        ...
        const updatedBooking = responseBody.data as BookingOutput_2024_08_13;
        expect(updatedBooking.location).toEqual(newPhone);
        expect(updatedBooking.metadata?.videoCallUrl).toEqual("");
      });
    });
  });
