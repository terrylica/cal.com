...
export class OAuth2Controller {
  ...
  async getClient(@Param("clientId") clientId: string): Promise<OAuth2ClientResponseDto> {
    try {
      ...
      this.errorHandler.handleClientError(err, "Failed to get client");
    }
  }
  ...
  @ApiOperation({
    ...
    description: "Exchanges an authorization code for access and refresh tokens. This is a legacy endpoint - prefer using POST /token instead.",
  })
  async exchange(
    @Param("clientId") clientId: string,
  ...
  @ApiOperation({
    ...
    description: "Refreshes an access token using a refresh token. This is a legacy endpoint - prefer using POST /token instead.",
  })
  async refresh(
    @Param("clientId") clientId: string,
