...
export class WorkflowService {
  ...
  static async scheduleLazySMSWorkflow({
    workflowTriggerEvent,
    workflowStepId,
    workflow,
    evt,
    ...
    const { uid: bookingUid } = evt;
    const log = logger.getSubLogger({
      prefix: [`[WorkflowService.scheduleLazySMSWorkflow]: bookingUid ${bookingUid}`],
    });
    ...
    if (
      ...
      log.debug(
        `Skipping lazy SMS reminder for workflow step ${workflowStepId} - scheduled date ${scheduledDate} is in the past`
      );
      return;
    }
    ...
    await tasker.create(
      "sendWorkflowSMS",
      { bookingUid, workflowReminderId: workflowReminder.id },
      {
        scheduledAt: scheduledDate,
        referenceUid: workflowReminder.uuid,
      }
