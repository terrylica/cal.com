...
<AccordionGroup>
  <Accordion title="Client not found (404)">
    ```json
    {
    ...
    ```
    No OAuth client exists with the provided `client_id`. Verify your client ID.
  </Accordion>
  <Accordion title="Client not approved (401)">
    ```json
    {
      "error": "unauthorized_client",
      "error_description": "client_not_approved"
    }
    ```
    The OAuth client has not been approved by a Cal.com admin yet. Wait for approval before using the client.
  </Accordion>
  <Accordion title="Mismatched redirect URI (400)">
    ```json
    {
    ...
    ```
    The `redirect_uri` does not match the one registered for the OAuth client. Ensure it matches exactly.
  </Accordion>
  <Accordion title="Authorization denied (redirect with error)">
    If the user cannot be authorized (e.g., invalid team slug), the user is redirected to the `redirect_uri` with an error:
    ```
